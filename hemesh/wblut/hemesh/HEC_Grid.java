...
public class HEC_Grid extends HEC_Creator {
	 ...
	 *            2D scalar function
	 * @param ui
	 *            starting u value for function
	 * @param vi
	 *            starting v value for function
	 * @param du
	 *            step size u for function
	 * @param dv
	 *            step size v for function
	 * 
	 * @return self
	 */
	...
	protected HE_Mesh createBase() {
		...
		final double odU = 1.0 / U;
		final double odV = 1.0 / V;
		final double[][] points = new double[(U + 1) * (V + 1)][3];
		final int[][] faces = new int[U * V][4];
		final double[][] uv = new double[(U + 1) * (V + 1)][2];
		int index = 0;
		if (values == null) {
			for (int j = 0; j < V + 1; j++) {
				for (int i = 0; i < U + 1; i++) {
		...
		fl.setVertices(points).setUV(uv).setUseUV(true).setFaces(faces)
