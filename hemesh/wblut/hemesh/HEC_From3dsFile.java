...
public class HEC_From3dsFile  extends HEC_Creator {
	...
	/**
	 * Instantiates a new hE c_ from3ds file.
	 */
	...
	protected HE_Mesh createBase() {
		if(path==null) return new HE_Mesh();
		try {
			loadScene(path);
		...
		for(Mesh3ds mesh3d:_scene.mMesh){
	...
	class Mesh3ds {
		...
		/** The m vertex. */
		WB_Point3d[] mVertex = new WB_Point3d[0];
	...
	class Decode3ds {
		 ...
		 * @param n the n
		 */
		public void printBytes(byte[] buf, int offset, int n) {
		 ...
		 * @return the string
		 */
		public String intToHex(int val, int digits) {
			char[] buf = new char[8];
		...
		public void printHead(int id, int length) {
			...
			if ((known == true) || (mDecLevel >= Scene3ds.DECODE_USED_PARAMS)) {
				println(name + "  id=0x" + intToHex(id, 4) + " length="
						+ length);
			}
		}
		...
		private class ChunkInfo {
			 ...
			 * @param name the name
			 */
			ChunkInfo(boolean known, int id, String name) {
				this.known = known;
				this.id = id;
				this.name = name;
			}
		...
		/** The Constant CHUNK_ROLL_TRACK_TAG. */
		private static final int CHUNK_M3DMAGIC = 0x4D4D,
				CHUNK_MDATA = 0x3D3D,
				...
				CHUNK_CAM_RANGES = 0x4720, CHUNK_KFDATA = 0xB000,
				CHUNK_KFSEG = 0xB008, CHUNK_OBJECT_NODE_TAG = 0xB002,
				CHUNK_NODE_ID = 0xB030, CHUNK_NODE_HDR = 0xB010,
				CHUNK_PIVOT = 0xB013, CHUNK_POS_TRACK_TAG = 0xB020,
				CHUNK_ROT_TRACK_TAG = 0xB021, CHUNK_SCL_TRACK_TAG = 0xB022,
				CHUNK_MORPH_TRACK_TAG = 0xB026, CHUNK_HIDE_TRACK_TAG = 0xB029,
				CHUNK_TARGET_NODE_TAG = 0xB004, CHUNK_CAMERA_NODE_TAG = 0xB003,
		...
		private final ChunkInfo[] mChunkInfo = {
				...
				new ChunkInfo(true, CHUNK_MDATA, "MDATA"),
				...
				new ChunkInfo(true, CHUNK_NAMED_OBJECT, "NAMED_OBJECT"),
				...
				new ChunkInfo(true, CHUNK_POINT_ARRAY, "POINT_ARRAY"),
				...
				new ChunkInfo(true, CHUNK_TEX_VERTS, "TEX_VERTS"),
				new ChunkInfo(true, CHUNK_SMOOTH_GROUP, "SMOOTH_GROUP"),
				new ChunkInfo(true, CHUNK_MESH_MATRIX, "MESH_MATRIX"),
				new ChunkInfo(false, 0x4165, "MESH_COLOR"),
				// new ChunkInfo(true, CHUNK_MESH_COLOR, "MESH_COLOR" ),
				new ChunkInfo(true, CHUNK_MESH_TEXTURE_INFO,
						"MESH_TEXTURE_INFO"),
				...
				new ChunkInfo(true, CHUNK_MAT_ENTRY, "MAT_ENTRY"),
				new ChunkInfo(true, CHUNK_MAT_NAME, "MAT_NAME"),
				...
				new ChunkInfo(true, CHUNK_KFDATA, "KFDATA"),
	...
	/**
	 * Exception class thrown by the {@link mri.v3ds.Scene3ds Scene3ds}
	 * constructors.
	 * 
	...
	public class Exception3ds extends java.lang.Exception {
		...
		/**
		 * Instantiates a new exception3ds.
		 *
		 * @param s the s
		 */
	...
	/**
	 * Toplevel class for parsing a 3ds-file and containing a 3D Studio vector
	 * animation.
	 * 
	 * The Scene3ds class constructors takes 3ds-file data as input, parses the
	 * data and builds a memory image of the relevant 3D vector animation data. <br>
	 * <br>
	 * A successfully created Scene3ds object contains the following "global"
	 * data:<br>
	...
	public class Scene3ds {
		/**
		 * Decode level constant. Decode all information. Unused parameters as
		 * hex bytes.
		 */
		public static final int DECODE_ALL = 3;

		/**
		 * Decode level constant. Decode all information except unused
		 * parameters.
		 */
		 ...
		 * 
		 * @param i
		 *            index into mesh array [0 ... meshes()-1]
		 * @return the specified mesh
		 */
		public Mesh3ds mesh(int i) {
			return (Mesh3ds) mMesh.get(i);
		}

		/**
		 * Construct (and decode) a Scene3ds object from a 3ds-file RAM image.
		 * The whole 3ds-file must be loaded beforhand into a
		 * <code>byte[]</code> array and passed as the <code>file_image</code>
		 * parameter.
		 * 
		 * @param file_image
		 *            file image of whole .3ds file
		 * @param decode
		 *            destination object for text decode
		 * @param level
		 *            decode level (DECODE_ALL, DECODE_USED_PARAMS,
		 *            DECODE_USED_PARAMS_AND_CHUNKS)
		 * @throws Exception3ds
		 *             in case of parsing problems
		 */
		public Scene3ds(byte[] file_image, TextDecode3ds decode, int level)
				throws Exception3ds {
			if (decode != null) {
		...
		/**
		 * Construct a Scene3ds object from a 3ds-file RAM image. The whole
		 * 3ds-file must be loaded beforhand into a <code>byte[]</code> array
		 * and passed as the <code>file_image</code> parameter.
		 * 
		 * @param file_image
		 *            file image of whole .3ds file
		 * @throws Exception3ds
		 ...
		 */
		public Scene3ds(File file, TextDecode3ds decode, int level)
				throws Exception3ds {
			if (decode != null) {
				mDecode = new Decode3ds(decode, level);
			}
		...
		/**
		 * Construct (and decode) a Scene3ds object from an InputStream
		 * containing 3ds-file data. The InputStream must contain exactly the
		 * 3ds-file data, nothing more and nothing less.
		 * 
		 * @param stream
		 *            InputStream containing 3ds-file data
		 * @param decode
		 *            destination object for text decode
		 * @param level
		 *            decode level (DECODE_ALL, DECODE_USED_PARAMS,
		 ...
		 *             in case of I/O or parsing problems
		 */
		public Scene3ds(InputStream stream, TextDecode3ds decode, int level)
				throws Exception3ds {
			if (decode != null) {
				mDecode = new Decode3ds(decode, level);
			}
		...
		/**
		 * Construct a Scene3ds object from an InputStream containing 3ds-file
		 * data. The InputStream must contain exactly the 3ds-file data, nothing
		 ...
		 *
		 * @return the byte
		 * @throws Exception3ds the exception3ds
		 */
		private byte readByte() throws Exception3ds {
			if (mFilePos >= mFileLength) {
				throw new Exception3ds(
						"Read out of bounds! File is probably corrupt.");
			}
		...
		private int skipBytes(int n) throws Exception3ds {
			if (n < 0) {
				throw new Exception3ds(
						"Negative chunk size! File is probably corrupt.");
		...
		private String read_NAME(int length) throws Exception3ds {
			...
			if (terminated == false) {
				throw new Exception3ds(
						"Name not terminated! File is probably corrupt.");
			}
		...
		private void read3DS() throws Exception3ds {
			...
			if (head.id != CHUNK_M3DMAGIC) {
				throw new Exception3ds(
						"Bad signature! This is not a 3D Studio R4 .3ds file.");
			}
		 ...
		 * @throws Exception3ds the exception3ds
		 */
		private void readColor(int chunk_len) throws Exception3ds {
			int chunk_end = filePos() + chunk_len;
		 ...
		 * @throws Exception3ds the exception3ds
		 */
		private void readSpotChunk(int chunk_len) throws Exception3ds {
			int chunk_end = filePos() + chunk_len;
		 ...
		 * @throws Exception3ds the exception3ds
		 */
		private WB_Point3d[] read_POINT_ARRAY() throws Exception3ds {
			int verts = readUnsignedShort();
			WB_Point3d[] v = new WB_Point3d[verts];

			for (int n = 0; n < verts; n++) {
				float x = readFloat();
				float z = readFloat();
				float y = readFloat();
				v[n] = new WB_Point3d(scale*x, scale*y,scale*z);
			}
		...
		private void read_SMOOTH_GROUP(Mesh3ds mes, int chunk_len)
			...
			if (entrys != mes.mFace.length) {
				throw new Exception3ds(
						"SMOOTH_GROUP entrys != faces. File is probably corrupt!");
			}
		}
		...
		/** The Constant CHUNK_ROLL_TRACK_TAG. */
		static final int CHUNK_COL_RGB = 0x0010,
				...
				CHUNK_M3DMAGIC = 0x4D4D, CHUNK_MDATA = 0x3D3D,
				CHUNK_MAT_ENTRY = 0xAFFF, CHUNK_MAT_NAME = 0xA000,
				CHUNK_MAT_AMBIENT = 0xA010, CHUNK_MAT_DIFFUSE = 0xA020,
				CHUNK_MAT_SPECULAR = 0xA030, CHUNK_MAT_SHININESS = 0xA041,
				CHUNK_MAT_TRANSPARENCY = 0xA050, CHUNK_MAT_MAP = 0xA200,
				CHUNK_MAT_MAPNAME = 0xA300, CHUNK_NAMED_OBJECT = 0x4000,
				CHUNK_N_TRI_OBJECT = 0x4100, CHUNK_POINT_ARRAY = 0x4110,
				CHUNK_TEX_VERTS = 0x4140, CHUNK_MESH_TEXTURE_INFO = 0x4170,
				CHUNK_MESH_MATRIX = 0x4160, CHUNK_MESH_COLOR = 0x4165,
				CHUNK_FACE_ARRAY = 0x4120, CHUNK_MSH_MAT_GROUP = 0x4130,
				CHUNK_SMOOTH_GROUP = 0x4150, CHUNK_N_LIGHT = 0x4600,
				CHUNK_LIT_SPOT = 0x4610, CHUNK_LIT_OFF = 0x4620,
				CHUNK_LIT_ATTENUATE = 0x4625, CHUNK_LIT_RAYSHAD = 0x4627,
				CHUNK_LIT_SHADOWED = 0x4630, CHUNK_LIT_LOCAL_SHADOW = 0x4640,
				CHUNK_LIT_LOCAL_SHADOW2 = 0x4641, CHUNK_LIT_SEE_CONE = 0x4650,
				CHUNK_LIT_SPOT_RECTANGULAR = 0x4651,
				CHUNK_LIT_SPOT_OVERSHOOT = 0x4652,
				CHUNK_LIT_SPOT_PROJECTOR = 0x4653,
				CHUNK_LIT_SPOT_RANGE = 0x4655, CHUNK_LIT_SPOT_ROLL = 0x4656,
				CHUNK_LIT_SPOT_ASPECT = 0x4657, CHUNK_LIT_RAY_BIAS = 0x4658,
				CHUNK_LIT_INNER_RANGE = 0x4659, CHUNK_LIT_OUTER_RANGE = 0x465A,
				CHUNK_LIT_MULTIPLIER = 0x465B, CHUNK_N_CAMERA = 0x4700,
				CHUNK_CAM_SEE_CONE = 0x4710, CHUNK_CAM_RANGES = 0x4720,
				CHUNK_KFDATA = 0xB000, CHUNK_KFSEG = 0xB008,
				CHUNK_OBJECT_NODE_TAG = 0xB002, CHUNK_NODE_ID = 0xB030,
				CHUNK_NODE_HDR = 0xB010, CHUNK_PIVOT = 0xB013,
				CHUNK_POS_TRACK_TAG = 0xB020, CHUNK_ROT_TRACK_TAG = 0xB021,
				CHUNK_SCL_TRACK_TAG = 0xB022, CHUNK_MORPH_TRACK_TAG = 0xB026,
				CHUNK_HIDE_TRACK_TAG = 0xB029, CHUNK_TARGET_NODE_TAG = 0xB004,
				CHUNK_CAMERA_NODE_TAG = 0xB003, CHUNK_FOV_TRACK_TAG = 0xB023,
	 ...
	 * @throws Exception3ds the exception3ds
	 */
	public void loadScene(String path) throws Exception3ds {
